...
elseif (isset($_POST['add_siswa'])) {
	...
	$idBidang = mysqli_real_escape_string($conn, $_POST['bidang']);
	...
	$sql_detail = "INSERT INTO detail_user (id_user,
		...
		?,
		?)";
	if ($nis === "" || $email === "" || $pwd === "" || $nama === "" || $idBidang == "" || $jk === "" || $sklh === "") {
...
// Tambah Bidang
elseif (isset($_POST['tambah_bidang'])) {
	$idBidang = mysqli_real_escape_string($conn, $_POST['tIdBidang']);
	$namaBidang = mysqli_real_escape_string($conn, $_POST['tNamaBidang']);


	$sql_detail = "INSERT INTO bidang(idBidang, namaBidang) VALUES (?,?)";
	if ($idBidang === "" || $namaBidang === "") {
		header("location:../lihatBidang&st=4");
	} else {
		if ($statement1 = $conn->prepare($sql_detail)) {
			$statement1->bind_param(
				"is",
				$idBidang,
				$namaBidang,
			...
			if ($statement1->execute()) {
				header("location:../lihatBidang&st=1");
			} else {
				header("location:../lihatBidang&st=2");
			}
		} else {
			header("location:../lihatBidang&st=2");
		}
	...
	$kepribadian = mysqli_real_escape_string($conn, $_POST['kepribadian']);
	$kedisplinan = mysqli_real_escape_string($conn, $_POST['kedisiplinan']);
	$keterampilan = mysqli_real_escape_string($conn, $_POST['keterampilan']);
	$kerja_sama = mysqli_real_escape_string($conn, $_POST['kerja_sama']);

	$sql_detail = "INSERT INTO tbl_nilai ( nama_peserta, kepribadian,kedisiplinan,keterampilan,kerja_sama) VALUES(?,?,?,?,?)";
	if ($nama_peserta === "" || $kepribadian === "") {
		header("location:../add_nilai&st=4");
	} else {
		if ($statement1 = $conn->prepare($sql_detail)) {
			$statement1->bind_param(
				"sssss",
			...
			if ($statement1->execute()) {
				header("location:../add_nilai&st=1");
			} else {
				header("location:../add_nilai&st=2");
			}
		} else {
			header("location:../add_nilai&st=2");
		}
...
elseif (isset($_POST['edit_bidang'])) {
	$id = mysqli_real_escape_string($conn, $_POST['uIdBidang']);
	$namaBidang = mysqli_real_escape_string($conn, $_POST['uNamaBidang']);


	$sql_detail = "UPDATE bidang SET idBidang=?, namaBidang=? WHERE idBidang='$id'";
	if ($namaBidang === "" || $id === "") {
		header("location:../lihatBidang&idBidang=$id&st=4");
	} else {
		if ($statement = $conn->prepare($sql_detail)) {
			...
			if ($statement->execute()) {
				header("location:../lihatBidang&idBidang=$id&st=1");
			} else {
				header("location:../lihatBidang&idBidang=$id&st=2");
			}
		} else {
			header("location:../lihatBidang&idBidang=$id&st=2");
		}
...
elseif (isset($_POST['edit_siswa'])) {
	...
	//$pwd = mysqli_real_escape_string($conn, sha1($_POST['pwd_cek']));
	$nama = mysqli_real_escape_string($conn, $_POST['nama']);
	$penempatan = mysqli_real_escape_string($conn, $_POST['bidang']);
	$jk = mysqli_real_escape_string($conn, $_POST['jk']);
	$sklh = mysqli_real_escape_string($conn, $_POST['sklh']);

	$sql_detail = "UPDATE detail_user SET nis_user=?, name_user=?,idBidang=?, sklh_user=?, jk_user=? WHERE id_user='$id'";
	if ($nis === "" || $id === "" || $nama === "" || $penempatan === "" || $jk === "" || $sklh === "") {
...
// Delete Bidang
elseif (isset($_GET['del_bidang'])) {
	$id = mysqli_real_escape_string($conn, $_GET['del_bidang']);
	$sql_d = "DELETE FROM bidang WHERE idBidang=?";
	if ($stmt = $conn->prepare($sql_d)) {
		$stmt->bind_param("i", $id);
		if ($stmt->execute()) {
			header("location:../lihatBidang&st=3");
		} else {
			header("location:../lihatBidang&st=5");
		}
	} else {
		header("location:../lihatBidang&st=5");
	}
