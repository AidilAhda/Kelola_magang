...
if (isset($_SESSION['pb'])) {
	...
	if ($conn->query("SELECT*FROM catatan WHERE id_user='$id_user'")->num_rows !== 0) {
		...
		while ($get_month = $query_month->fetch_assoc()) {
			...
			if ($cek !== 0) {
				echo "<h4 class='sub-header'><strong>Laporan:</strong> $get_user[name_user] - <i>$month</i> </h4>";
		...
		echo "<div class='alert alert-warning'><strong>Tidak ada Laporan untuk ditampilkan.</strong></div>";
	}
} elseif (isset($_SESSION['sw'])) {
	echo "<h3 class='sub-header'>Laporanku <button type='button' onclick=\"window.location.href='tambah_catatan';\" class='btn btn-success'>Tambah</button> </h3>";
